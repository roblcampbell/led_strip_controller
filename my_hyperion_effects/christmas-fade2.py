...
class ChristmasLightsScene(LedScene):
    ...
    allRandom = hyperion.args.get('randomType', 'string') == 'full'
    fadeSpeed1 = hyperion.args.get('fadeSpeed1', 3.0)
    fadeRandom = hyperion.args.get('fadeRandom', 1.0)
    speed = hyperion.args.get('velocity', 0)

    while True:
        color = colors[colorIndex]
        fadeSeconds = fades[colorIndex]
        if allRandom:
            fadeSeconds = random.uniform(fadeSpeed1 - fadeRandom, fadeSpeed1 + fadeRandom)
        bulb = FadingXmasBulb(color, fadeSeconds)
        bulb.baseLocation = ledIndex
        bulb.bulbSize = bulbSize
        bulb.velocity = speed
        LedScene.ledObjects.append(bulb)
        colorIndex = (colorIndex + 1) % len(colors)
        ledIndex += (bulbSize + gapWidth)
        if (ledIndex >= hyperion.ledCount - 10):
            break
